...
@Tag(name = "Delete-Admin - Uploads", description = "Upload ბაჩების მართვა (წაშლა assessmentNo და თარიღით)")
public class UploadAdminController {
    ...
    @GetMapping("/uploads")
    @Operation(
            summary = "ნახე uploads (ფილტრით assessmentNo და/ან დროით)",
            description = "თუ assessmentNo არ მიუთითე — ყველა upload ჩამოიტანს. თუ მიუთითე + from/to — [from, to) დიაპაზონია."
    )
    public List<UploadDTO> listUploads(
            @RequestParam(required = false)
            @Parameter(example = "9") Integer assessmentNo,
